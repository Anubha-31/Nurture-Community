...
public class Queries {
	...
	public static String foodListsql = "select food_details.food_id,food_details.address1,food_details.address2,food_details.city,food_details.country,food_details.item_description,food_details.item_name,food_details.location_change,food_details.numberof_packets,food_details.pickup_time,food_details.uploaded_picture from users LEFT JOIN food_details on food_details.id = users.id where users.id = ( select DISTINCT id from users where users.emailaddress = ? ) order by food_details.numberof_packets desc";
